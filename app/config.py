...
if "openrouter" in _RAW_LLM_BASE_URL.lower():
    ...
    LLM_API_KEY = _env("LLM_API_KEY") or _env("OPENROUTER_API_KEY") or _env("GROQ_API_KEY") or _env("NVIDIA_API_KEY")
...
LLM_MODEL = _env("LLM_MODEL") or _env("OPENROUTER_MODEL") or _env("NVIDIA_MODEL") or "deepseek/deepseek-v4-flash-0731:free"
...
LLM_FALLBACK_MODELS = [
    m.strip() for m in (_env("LLM_FALLBACK_MODELS") or "openrouter/free").split(",") if m.strip()
]
...
def startup_report() -> list:
    ...
    if not LLM_API_KEY:
        problems.append("FATAL: no LLM API key is set (LLM_API_KEY, OPENROUTER_API_KEY, GROQ_API_KEY, or NVIDIA_API_KEY) — the agent cannot think.")
